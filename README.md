<h1 align="center">
  <br>
  <img src="icon_big.png" alt="WinToLin" width="150">
  <br>
  WinToLin
  <br>
</h1>

<h4 align="center">Switch from Windows to Linux safely, simply, and hands-free.</h4>

<p align="center">
  <a href="#features">Features</a> •
  <a href="#how-it-works">How It Works</a> •
  <a href="#contributing">Contributing</a> •
  <a href="#license">License</a>
</p>

> **Make switching to Linux easier than reinstalling Windows.**

---

## Current State


<h3 align="center">Currently supported distros to migrate to</h3>
<table align="center">
  <thead>
    <tr>
      <th>Distro</th>
      <th>File Transfer</th>
      <th>File Mapping</th>
      <th>App Transfer</th>
      <th>App Config Transfer</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>Ubuntu</td>
      <td>✅ (Since 23.04.2026)</td>
      <td>✅ (Since 27.05.2026)</td>
      <td>🟡 (Since 23.04.2026)</td>
      <td>❌</td>
    </tr>
    <tr>
      <td>Fedora</td>
      <td>✅ (Since 21.05.2026)</td>
      <td>✅ (Since 27.05.2026)</td>
      <td>🟡 (Since 26.05.2026)</td>
      <td>❌</td>
    </tr>
    <tr>
      <td>Mint</td>
      <td>❌</td>
      <td>❌</td>
      <td>❌</td>
      <td>❌</td>
    </tr>
    <tr>
      <td>Other Distros</td>
      <td colspan=4 align="center">Will come</td>
    </tr>
  </tbody>
</table>


<table align="center">
  <thead>
    <tr>
      <td>Icon</td>
      <td>Meaning</td>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>✅</td>
      <td>Fully Working</td>
    </tr>
    <tr>
      <td>🟡</td>
      <td>Lists need to be extended (supported apps, app config paths, ...)</td>
    </tr>
    <tr>
      <td>🟠</td>
      <td>Partialy Implemented</td>
    </tr>
    <tr>
      <td>❌</td>
      <td>Not Implemented</td>
    </tr>
  </tbody>
</table>


* **Requirements :** xorriso installed and setup with a path variable in the system
 
## Features

* **System Analysis:** Detects installed apps and checks Linux compatibility.
* **Auto-Backup:** Secures your personal files to prevent data loss.
* **Media Creator:** Downloads and builds an automated Linux installer.
* **Hands-Free Setup:** Automatically restores files and installs alternative apps post-installation.

## How It Works

1. **Analyze** your current system.
2. **Choose** your preferred Linux distribution.
3. **Backup** your files and configuration.
4. **Prepare** the bootable installation media.
5. **Install** Linux and let WinToLin **Restore** everything automatically.

## Demo Video
https://github.com/user-attachments/assets/eaef0d00-9c2e-41cb-abe1-394ea9ccd5f3



## Contributing

Contributions are welcome! Help us expand the app compatibility database, improve the UI/UX, or add more supported distributions by opening an issue or pull request.

## License

This project is open-source and free for everyone under the GPL 2 License.
